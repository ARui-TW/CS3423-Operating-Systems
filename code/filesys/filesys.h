...
public:
    ...
    OpenFileId OpenAFile(char *name)
    {
        ...
        for (OFI = 0; OFI < 20; OFI++)
            if (OpenFileTable[OFI] == NULL) break;

        if (OFI < 0 || OFI >= 20)
            return -1;

        OpenFileTable[OFI] = OF;
        return OFI;
    }
    ...
    int WriteFile(char *buffer, int size, OpenFileId id)
    {
        ...
        OpenFile *OF = OpenFileTable[id];
        int numWritten = OF->Write(buffer, size);
        return numWritten;
    }
    ...
    int ReadFile(char *buffer, int size, OpenFileId id)
    {
        ...
        OpenFile *OF = OpenFileTable[id];
        int numWritten = OF->Read(buffer, size);
        return numWritten;
    }
